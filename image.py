...
def resize_image_opencv(input_image, output_image, resize_to_width, resize_to_height):
    ...
    scaled_img = cv.resize(img, (w, h), interpolation=interp)
    padded_img = cv.copyMakeBorder(scaled_img,0,pad_bottom,0,pad_right,borderType=cv.BORDER_CONSTANT,value=[0,0,0])

    cv.imwrite(output_image, padded_img)
...
def resize_all_opencv(input_path, output_path, resize_to_width, resize_to_height, allowed_image_extensions):
    ...
    for image in images:
        ...
        padded_img = cv.copyMakeBorder(
            scaled_img,0,pad_bottom,0,pad_right,borderType=cv.BORDER_CONSTANT,value=[0,0,0])
        
        cv.imwrite(os.path.join(output_path, filename_ext[0]+"_resized."+filename_ext[1]), padded_img)
    ...
    for root, dirs, files in os.walk(path):
        for file in files:
            if file.endswith(tuple(config["files"]["allowed_image_extensions"])):
                ...
                cv.imshow(config["opencv"]["window_name"], img)
                cv.waitKey(0)
